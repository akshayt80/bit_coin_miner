defmodule BitCoin do
    def start_mining(map, k \\ 3) do
        parent = self()
        # Spawns the function which mines the bit coin
        spawn fn -> mine(k, parent) end
        # receiving message from child
        map = receive do
            {:good_coin, coin_details} -> check_coin_validity(elem(coin_details, 0), elem(coin_details, 1), map)
        end
        start_mining(map, k)
    end
    ...
    defp check_coin_validity(coin_str, coin_hash, map) do
